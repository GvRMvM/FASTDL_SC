// Base Script for Pubsec Weapons
// Based off the base script from KernCore's CS 1.6 Weapons
// Modified by Meryilla

namespace TSBASE
{ //Namespace start

bool ShouldUseCustomAmmo = false; // true = Uses custom ammo values; false = Uses SC's default ammo values.

enum TS_SELECTFIRE_OPTIONS // Select fire options
{
	SELECTFIRE_SEMI = 0,
	SELECTFIRE_3XBURST,
	SELECTFIRE_AUTO,
...
//Model files
//Sound files
string FIREMODE_SELECT_S	= "weapons/357_chamberin.wav";
...
mixin class WeaponBase
{
	...
	void PlayDeploySound( string szDeploySound )
	{
		g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_VOICE, szDeploySound, VOL_NORM, ATTN_NORM, 0, PITCH_NORM );
	}
	...
	void DisplayFiremode() // displays the fire mode
		{
			switch( WeaponSelectFireMode )
			{
				...
				case SELECTFIRE_SEMI:
					g_EngineFuncs.ClientPrintf( m_pPlayer, print_center, "Switched to Semi-Automatic\n" );
					break;

				case SELECTFIRE_PUMP:
					g_EngineFuncs.ClientPrintf( m_pPlayer, print_center, "Switched to Pump-Action\n" );
					break;
			}
		}	
	
	void ChangeFireMode( float flTimer, int FirstFireMode, int SecondFireMode, int ThirdFireMode = 100  ) // switch fire modes (third fire mode is optional)
	{
		...
		else if( (m_pPlayer.pev.button & IN_USE != 0) && (m_pPlayer.pev.button & IN_RELOAD != 0) )
		{
			if( m_useTimer < g_Engine.time )
			{
				...
				else if( WeaponSelectFireMode == SecondFireMode && ThirdFireMode != 100)
				{
					WeaponSelectFireMode = ThirdFireMode;
				}
				else if( WeaponSelectFireMode == SecondFireMode && ThirdFireMode == 100)
				{
				...
				DisplayFiremode();
				g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_VOICE, FIREMODE_SELECT_S, VOL_NORM, ATTN_NORM, 0, PITCH_NORM );
	...
	void PunchAngle( Vector& in vPunch ) // aimed recoil
	{
		...
		m_pPlayer.pev.punchangle.x = vPunch.x;
		m_pPlayer.pev.punchangle.y = vPunch.y;
		m_pPlayer.pev.punchangle.z = vPunch.z;
	...
	// TS Kickback Recoil. Same as the normal kickback function but setting the recoil to veer explicitly to the left or right
	void KickBackTS( float up_base, float lateral_base, float up_modifier, float lateral_modifier, float up_max, float lateral_max, bool fIsRightLeaning = false )
	{
