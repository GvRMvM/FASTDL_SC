...
class WeaponCustomBase : ScriptBasePlayerWeaponEntity
{
	...
	void RegenAmmo(int ammoType)
	{
		if (m_pPlayer is null)
			return;
		int ammoLeft = m_pPlayer.m_rgAmmo(ammoType);
		int maxAmmo = m_pPlayer.GetMaxAmmo(ammoType);
		...
		m_pPlayer.m_rgAmmo(ammoType, ammoLeft);
	}
	...
	bool PlayEmptySound(weapon_custom_shoot@ opts)
	{
		...
		snd.play(m_pPlayer, CHAN_WEAPON);
	...
	bool Deploy(bool skipDelay)
	{
		...
		baseMoveSpeed = m_pPlayer.pev.maxspeed;
		
		settings.deploy_snd.play(m_pPlayer, CHAN_VOICE);
		...
		for (uint i = 0; i < keys.length(); i++)
		{
			...
			if (isCustomAmmo and ammo.custom_ammo_type == settings.primary_ammo_type)
				m_pPlayer.SetMaxAmmo(ammo.custom_ammo_type, ammo.max_ammo);
		}
	...
	void applyPlayerSpeedMult()
	{
		...
		m_pPlayer.pev.maxspeed = baseMoveSpeed*mult;
		if (m_pPlayer.pev.maxspeed == 0)
			m_pPlayer.pev.maxspeed = 0.000000001; // 0 just resets to default
	...
	void Holster(int iSkipLocal = 0) 
	{
		...
		{
			active_opts.windup_snd.stop(m_pPlayer, CHAN_VOICE);
			active_opts.wind_down_snd.stop(m_pPlayer, CHAN_VOICE);
			active_opts.windup_loop_snd.stop(m_pPlayer, CHAN_VOICE);
			active_opts.hook_snd.stop(m_pPlayer, CHAN_VOICE);
			active_opts.hook_snd2.stop(m_pPlayer, CHAN_VOICE);
		}
		
		for (uint i = 0; i < ubeams.length(); i++)
			g_EntityFuncs.Remove(ubeams[i]);
		
		m_pPlayer.pev.maxspeed = baseMoveSpeed;
		if (settings.pev.spawnflags & FL_WEP_NO_JUMP != 0)
	...
	void ShootOneBullet()
	{
		...
		if (reviveOnly)
		{
			CBaseEntity@ revTarget = getReviveTarget(tr.vecEndPos, REVIVE_RADIUS, m_pPlayer, active_opts);
			...
			if( tr.pHit !is null )
			{
				...
				{					
					...
					if (playDefaultMeleeSnd)
					{
						...
						WeaponSound@ snd = active_opts.getRandomMeleeHitSound();
						if (snd !is null)
							snd.play(m_pPlayer, CHAN_VOICE, volume);
					}
					
					EHandle h_plr = m_pPlayer;
					EHandle h_ent = pHit;
	...
	CBaseEntity@ ShootCustomProjectile(string classname, Vector ori, Vector vel, Vector angles)
	{
		...
		@shootEnt.pev.owner = m_pPlayer.edict(); // do this or else crash	
	...
	CBaseEntity@ ShootProjectile()
	{
		...
		if (options.type == PROJECTILE_ARGRENADE)
			@nade = g_EntityFuncs.ShootContact( m_pPlayer.pev, projectile_ori, projectile_velocity );
		else if (options.type == PROJECTILE_BANANA)
			@nade = g_EntityFuncs.ShootBananaCluster( m_pPlayer.pev, projectile_ori, projectile_velocity );
		else if (options.type == PROJECTILE_BOLT)
			ShootCustomProjectile("crossbow_bolt", projectile_ori, projectile_velocity, m_pPlayer.pev.v_angle);
		else if (options.type == PROJECTILE_HVR)
			ShootCustomProjectile("hvr_rocket", projectile_ori, projectile_velocity, m_pPlayer.pev.v_angle);
		else if (options.type == PROJECTILE_SHOCK)
			ShootCustomProjectile("shock_beam", projectile_ori, projectile_velocity, m_pPlayer.pev.v_angle);
		else if (options.type == PROJECTILE_HORNET)
			ShootCustomProjectile("playerhornet", projectile_ori, projectile_velocity, m_pPlayer.pev.v_angle);
		else if (options.type == PROJECTILE_DISPLACER)
			@shootEnt = g_EntityFuncs.CreateDisplacerPortal( projectile_ori, projectile_velocity, m_pPlayer.edict(), 250, 250 );
		else if (options.type == PROJECTILE_GRENADE)
			@nade = g_EntityFuncs.ShootTimed( m_pPlayer.pev, projectile_ori, projectile_velocity, grenadeTime );
		else if (options.type == PROJECTILE_MORTAR)
			@nade = g_EntityFuncs.ShootMortar( m_pPlayer.pev, projectile_ori, projectile_velocity );
		else if (options.type == PROJECTILE_RPG)
		{
			@shootEnt = g_EntityFuncs.CreateRPGRocket(projectile_ori, m_pPlayer.pev.v_angle, m_pPlayer.edict());
			shootEnt.pev.velocity = shootEnt.pev.velocity + projectile_velocity;
		}
		else if (options.type == PROJECTILE_WEAPON)
			@shootEnt = ShootCustomProjectile("custom_projectile", projectile_ori, projectile_velocity, m_pPlayer.pev.v_angle);
		else if (options.type == PROJECTILE_TRIPMINE)
		{
			...
			@shootEnt = ShootCustomProjectile(options.entity_class, projectile_ori, projectile_velocity, m_pPlayer.pev.v_angle);
		else
	...
	bool AttackMonster(Vector vecSrc, TraceResult tr)
	{
		...
		float baseDamage = applyDamageModifiers(attackDamage, ent, m_pPlayer, active_opts);
		...
		else
		{
			...
			ent.TraceAttack(m_pPlayer.pev, baseDamage, attackDir, tr, dmgType);
			g_WeaponFuncs.ApplyMultiDamage(ent.pev, m_pPlayer.pev);
		}
	...
	array<BeamShot> CalcBeamShots()
	{
		...
		edict_t@ traceEnt = m_pPlayer.edict();
		
		for (int i = 0; i < active_opts.beam_ricochet_limit+1; i++)
		{			
			TraceResult tr;
			edict_t@ ignoreEnt = i == 0 ? m_pPlayer.edict() : self.edict();
				...
				for (int k = 0; k < MAX_BEAMS; k++)
				{
					...
					if (!beams[k][i])
					{
						...
						if (i == 0)
						{
							...
							ricobeam.SetEndEntity(m_pPlayer);
	...
	void CancelBeam()
	{
		...
		if (beamWasActive and minBeamTime == 0)
		{
			if (lastShootSnd !is null)
				lastShootSnd.stop(m_pPlayer, CHAN_VOICE);
			active_opts.hook_snd.stop(m_pPlayer, CHAN_VOICE);
			active_opts.hook_snd2.play(m_pPlayer, CHAN_VOICE);
	...
	void CreateUserBeam(weapon_custom_user_effect@ effect)
	{
		...
		switch (effect.beam_mode)
		{
			case UBEAM_ATTACH_ATTACH:
				...
				ubeam.SetStartEntity(m_pPlayer);
				ubeam.SetStartAttachment(int(effect.beam_start.x));
				ubeam.SetEndEntity(m_pPlayer);
	...
	void DetonateSatchels()
	{
		...
		//g_EntityFuncs.UseSatchelCharges(m_pPlayer.pev, SATCHEL_DETONATE);
		...
		do {
			...
			{				
				...
				if (owner.entindex() == m_pPlayer.entindex())
					ent.Use(m_pPlayer, m_pPlayer, USE_TOGGLE, 0);
			}
		} while (ent !is null);
	}
	...
	void WindupThink()
	{
			...
			if (shouldWindDown)
			{
				...
				if (!windingDown)
				{
					...
					if (snd.file.Length() > 0 and playSoundOnce)
						snd.play(m_pPlayer, CHAN_VOICE, 1.0f);
				...
				else // winding down
				{
					...
					else
					{
						...
						bool playSoundOnce = snd.options !is null and snd.options.pev.renderfx == 2;
						if (snd.file.Length() > 0 and !playSoundOnce)
							snd.play(m_pPlayer, CHAN_VOICE, 1.0f, newPitch, SND_CHANGE_PITCH);
					}
		...
		{
			bool longJumping = m_pPlayer.m_Activity == ACT_LEAP;
			if (settings.player_anims == ANIM_REF_CROWBAR)
			{
				bool correctAnim = m_pPlayer.pev.sequence == 25 or m_pPlayer.pev.sequence == 26;
				if (m_pPlayer.m_fSequenceFinished or (!correctAnim and !longJumping)) 
				{
			...
			{
				bool correctAnim = m_pPlayer.pev.sequence == 33 or m_pPlayer.pev.sequence == 34;
				if (m_pPlayer.m_fSequenceFinished or (!correctAnim and !longJumping)) 
				{
					// Manually set wrench windup loop animation
					m_pPlayer.m_Activity = ACT_RELOAD;
					m_pPlayer.pev.frame = 0;
					m_pPlayer.pev.sequence = 34;
					m_pPlayer.ResetSequenceInfo();
				}
			}
			...
			if (!windupLoopEntered and active_opts.windup_anim_loop != -1)
			{
				...
				{
					active_opts.windup_loop_snd.play(m_pPlayer, CHAN_VOICE);	
			...
			if (windingDown)
			{
				...
				{
					active_opts.wind_down_snd.stop(m_pPlayer, CHAN_VOICE);
					active_opts.windup_snd.play(m_pPlayer, CHAN_VOICE, 1.0f, active_opts.windup_pitch_start);	
				}
			...
			else if (timePassed < active_opts.windup_time)
			{
				...
				if (newPitch != active_opts.windup_pitch_start)
					active_opts.windup_snd.play(m_pPlayer, CHAN_VOICE, 1.0f, newPitch, SND_CHANGE_PITCH);
			...
			{		
				...
				bool dontShootOnWindup = windupOnly and (m_pPlayer.pev.button & IN_ATTACK == 0) 
				...
				if (!dontShootOnWindup)
				{
					...
					if (active_opts.windup_action == WINDUP_SHOOT_ONCE or onceHeld)
					{
						...
						if (active_opts.windup_snd.file.Length() > 0)
							g_SoundSystem.StopSound( m_pPlayer.edict(), CHAN_VOICE, active_opts.windup_snd.file);
						DoAttack(true);
						applyPlayerSpeedMult();
					}
		...
		if (reloading > 0)
		{
			...
			if (nextReload < g_Engine.time or responsiveCancel)
			{
				...
				else
				{
					...
					settings.reload_snd.play(m_pPlayer, CHAN_VOICE);
		...
		if (beam_active)
		{
			if (AllowedToShoot(active_opts) and windupHeld and minBeamTime == 0 or first_beam_shoot) 
			{
				...
				if (minBeamTime == 0 and !hookAnimStarted and hookAnimStartTime <= g_Engine.time)
				{
					...
					active_opts.hook_snd.play(m_pPlayer, CHAN_VOICE);
	...
	void EjectShell()
	{
		if (active_opts.shell_delay > 0)
			active_opts.shell_delay_snd.play(m_pPlayer, CHAN_ITEM);
		...
		Vector shellOri = m_pPlayer.GetGunPosition() + ofs;
		Vector shellVel = m_pPlayer.pev.velocity + vel;
	...
	void AttackEffects(bool windupAttack=false)
	{
		...
		m_pPlayer.pev.velocity = m_pPlayer.pev.velocity + kickVel;
		...
		if (windupAttack and settings.player_anims == ANIM_REF_CROWBAR)
		{
			...
			m_pPlayer.m_Activity = ACT_RELOAD;
			m_pPlayer.pev.frame = 0;
			m_pPlayer.pev.sequence = 27;
			m_pPlayer.ResetSequenceInfo();
			//m_pPlayer.pev.framerate = 0.5f;
		}
		...
		// recoil
		m_pPlayer.pev.punchangle.x = -Math.RandomFloat(active_opts.recoil.x, active_opts.recoil.y);
		m_pPlayer.pev.punchangle.y = 0;//Math.RandomLong(-180, 180);
		m_pPlayer.pev.punchangle.z = 0;//Math.RandomLong(-180, 180);
		...
		if (!healedTarget and !harmlessWep)
		{
			...
			m_pPlayer.m_iExtraSoundTypes = bits_SOUND_COMBAT;//bits_SOUND_DANGER;
		...
		{
			Vector lpos = m_pPlayer.pev.origin + g_Engine.v_forward * 50;
		...
		if (noSndOverlap)
		{
			if (lastCooldownSnd !is null)
				lastCooldownSnd.stop(m_pPlayer, CHAN_VOICE);
		}
	...
	{	
		...
		if (reviveOnly)
		{
			CBaseEntity@ revTarget = getReviveTarget(tr.vecEndPos, REVIVE_RADIUS, m_pPlayer, opts);
			if (revTarget !is null)
			{
	...
	bool ReloadContinuous(bool doReload=true)
	{
		...
		if (doReload)
		{
			...
			ammoLeft -= reloadAmt;
			m_pPlayer.m_rgAmmo( active_ammo_type, ammoLeft);
		}
	...
	void DepleteAmmo(int amt)
	{
		...
		if (self.m_iClip > 0 and shouldUseClip) 
			...
			m_pPlayer.m_rgAmmo( active_ammo_type, Math.max(0, AmmoLeft(active_ammo_type)-amt));
			
		if( m_pPlayer.m_rgAmmo(active_ammo_type) <= 0 )
	...
	int AmmoLeft(int ammoType)
	{
		...
		return Math.max(0, m_pPlayer.m_rgAmmo( ammoType ));
	}
	...
	void ShowLaser()
	{
		if (!laser_spr)
		{
			CSprite@ dot = g_EntityFuncs.CreateSprite( settings.laser_sprite, m_pPlayer.pev.origin, true, 10 );
	...
	bool AllowedToShoot(weapon_custom_shoot@ opts)
	{
		...
		if( m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD and !opts.can_fire_underwater())
		{
			emptySound = true;
			canshoot = false;
		}
	...
	void CommonAttack(int attackNum)
	{
		...
		if (fireAct != FIRE_ACT_SHOOT)
		{
			...
			{ 
				...
				if (settings.player_anims == ANIM_REF_BOW)
					m_pPlayer.set_m_szAnimExtension(self.m_fInZoom ? "bowscope" : "bow");
				if (settings.player_anims == ANIM_REF_SNIPER)
					m_pPlayer.set_m_szAnimExtension(self.m_fInZoom ? "sniperscope" : "sniper");
	...
	void Reload()
	{
			...
			self.m_iClip < settings.clip_size() and AmmoLeft(active_ammo_type) >= settings.reload_ammo_amt)
		{
			...
			nextShootTime = nextReload;
			self.pev.nextthink = g_Engine.time;
			settings.reload_start_snd.play(m_pPlayer, CHAN_VOICE);
			CancelZoom();
			windupHeld = false;
			return;
		}
